...
public class ItemBuddhistrelics extends RelicItem implements IAdvancementRequirement {
    ...
    public void onItemUpdate(LivingEvent.LivingTickEvent event) {
        if (event.getEntity() instanceof Player && !event.getEntity().level().isClientSide) {
            ...
            for (int i = 0; i < player.getInventory().getMaxStackSize(); i++) {
                ...
                if (ItemSwordRelic.isRelicItem(stack)) {
    ...
    public static void relicInit(ItemStack stack) {
        if (!ItemNBTHelper.getBoolean(stack, TAG_HASDATA, false)) {
            ...
            handler.setStackInSlot(0, new ItemStack(ModItems.excaliber.get()));
            handler.setStackInSlot(1, new ItemStack(ModItems.infinitewine.get()));
            handler.setStackInSlot(2, new ItemStack(ModItems.failnaught.get()));
    ...
    public static ItemStack expired(ItemStack morphstack) {
        if (ItemNBTHelper.getBoolean(morphstack, TAG_MORPHING, false)) {
            ...
            ItemStack budd = new ItemStack(ModItems.buddhistrelics.get());
    ...
    public static ItemStack relicShift(ItemStack heldstack) {
        if (heldstack.getItem() == ModItems.buddhistrelics.get()) {
            relicInit(heldstack);
