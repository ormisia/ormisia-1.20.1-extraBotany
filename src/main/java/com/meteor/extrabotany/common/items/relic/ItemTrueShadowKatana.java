...
public class ItemTrueShadowKatana extends ItemSwordRelic {
    ...
    public void attackEntity(LivingEntity player, Entity target) {
        ...
        if (list.size() == 0) {
            targetpos = target == null ? Helper.PosToVec(raytraceFromEntity(player, 64F, true).getBlockPos()).add(0, 1, 0) : target.position().add(0, 1, 0);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Vec3 axis1 = VecHelper.rotate(axis.scale(div * 2.5 + 2), rot, look);
            if (axis1.y < 0) {
